...
WITH cte (
	...
	[BedDays], [independent_care], [flag_qtr]
) 
AS (
	...
	FROM (
		...
		--Procedure lookup
		INNER JOIN (
			SELECT d.Year, d.Fin_month, d.org_code, d.hospital_no, d.id_pro 
			FROM (
				...
				SELECT *, ROW_NUMBER() OVER(PARTITION BY lu.org_code, lu.hospital_no ORDER BY priority) AS row_num
		...
		ON ip.ProviderCode = lu.org_code
		AND ip.Year = lu.Year
		AND ip.Fin_Month = lu.Fin_month
		AND ip.SLAM_HospitalNo = lu.hospital_no
	...
	--Join to get the most recent Qtr to flag incomplete data
	LEFT JOIN (
		SELECT TOP(1)
		year AS qf_year,
		CASE
			WHEN Fin_month IN (1,2,3) THEN 'Q1'
			WHEN Fin_month IN (4,5,6) THEN 'Q2'
			WHEN Fin_month IN (7,8,9) THEN 'Q3'
			WHEN Fin_month IN (10,11,12) THEN 'Q4'
			END AS [qf_Qtr]
		FROM [Data_Lab_NCL].[dbo].[apc_day_prolu]
		ORDER BY Year DESC, Fin_month DESC
	) qf
	ON qf.qf_year = agg.Year
	AND qf.qf_Qtr = agg.Qtr

	GROUP BY		[Fin_Month], [Year], [Qtr],
					-- For Org name
					[org_code], [org_name], [shorthand],
					-- For POD category AND Booked Type
					[POD],
