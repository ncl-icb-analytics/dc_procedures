...
/****** Object:  View [BLUE\KealeyJ].[apc_day_daycaserate_vw]    Script Date: 04/12/2023 09:41:34 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [BLUE\KealeyJ].[apc_day_daycaserate_vw]
AS
...
AS (
	...
	FROM (
		...
		ON ip.SLAM_HospitalNo = fss.[spell.hospital_provider_spell_number]
		AND ip.ProviderCode = fss.[spell.commissioning.service_agreement.provider_derived]

		--For episode info (To get dominant episode)
		INNER JOIN [Data_Store_SUS_Unified].[APC].[spell.episodes] fsse
		ON fss.PRIMARYKEY_ID = fsse.PRIMARYKEY_ID
